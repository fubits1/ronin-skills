...
> **Mixed runtimes.** `nogrep.mjs` and `no-honest.mjs` are zero-dependency Node scripts that run
...
`force-plan-mode.sh` drives the `agent:plan` flow, and `nogrep.mjs`'s block messages point at
...
```
node plugins/hooks/hooks/tests/run-nogrep-tests.mjs    # CI: nogrep.mjs functional contract
node plugins/hooks/hooks/tests/redteam-nogrep.mjs      # CI: nogrep.mjs adversarial / edge-case battery
node plugins/hooks/hooks/tests/mock-install-nogrep.mjs # CI: nogrep.mjs install-wiring proof
