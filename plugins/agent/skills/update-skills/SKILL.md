...
- **Target marketplace**: usually `<repo>/plugins/<plugin>/skills/<skill>/`. Plugin split by domain (e.g. `agent` for cross-cutting agent discipline; `frontend` for web/CSS/tests; `svelte-5` for Svelte components). Enforcement hooks may live in a dedicated hooks plugin for optional install (e.g. `plugins/hooks/hooks/no-bash.mjs` paired with the `agent:no-bash` skill), kept in the same marketplace.
...
- **Reviewing a skill measures coverage against the failure log, not authoring polish.** Asked whether a skill is good enough, answer with a fraction: enumerate every failure in the log, mark each GUARDED (quote the catching line) / PARTIAL / GAP, report the ratio. Clean prose and a passing lint say nothing about whether it stops the frauds it exists to stop; guarding 3 of 159 failures is not "fine".
...
- **The "no paraphrase" rule propagates to reviewer subagents.** When you dispatch an agent to review or check, tell it to INVOKE the governing skill itself, never your summary; a subagent reviewing against a paraphrase rubber-stamps what the real skill would catch. And treat its returned verdict as a summary, not proof: re-derive the evidence yourself before trusting it.
